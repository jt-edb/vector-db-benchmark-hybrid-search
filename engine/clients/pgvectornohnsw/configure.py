...
from engine.clients.pgvectornohnsw.config import PGVECTOR_COLLECTION_NAME


class PgvectorNoHnswConfigurator(BaseConfigurator):
    ...
    def recreate(self, dataset: Dataset, collection_params):
        ...
        with self.client.cursor() as cur:
            ...
            # Indexes on filters only
            create_idx_stmt = f"""
                CREATE INDEX ON {PGVECTOR_COLLECTION_NAME} USING gin (labels)
                """
            cur.execute(create_idx_stmt)
            create_idx_stmt = f"""
                CREATE INDEX ON {PGVECTOR_COLLECTION_NAME} USING btree (submitter)
                """
            cur.execute(create_idx_stmt)
            create_idx_stmt = f"""
                CREATE INDEX ON {PGVECTOR_COLLECTION_NAME} USING btree (update_date_ts)
                """
            cur.execute(create_idx_stmt)
